...
1. Given a training set $\mathcal{X}$ such that $x_i \in \mathbb{R}^d$, 
$0 < i \leq n$, for each $i$, construct a $k$NN model from 
$\mathcal{X} \setminus \{x_i\}$ to obtain $\hat{y}_i$.
...
cifar10.edited.df %>% 
  ...
  geom_errorbar(aes(x = train_size, 
                    ...
                    colour = 'not edited')) + 
  geom_errorbar(aes(x = train_size, 
                    ymin = mean.acc.edited - 2 * sd.acc.edited, 
                    ymax = mean.acc.edited + 2 * sd.acc.edited, 
                    colour = 'edited')) + 
