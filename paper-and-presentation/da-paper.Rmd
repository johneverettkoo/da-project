...
may result in better model performance. For each class, we used $k$-means 
clustering with $k = 10$ and drew training data maintaining cluster balance for 
...
```{r cifar10_clustered, fig.cap = 'Diversity experiment on CIFAR-10 data, drawing from clusters.'}
...
cifar10.val.from.train.df %>% 
  dplyr::bind_rows(
    cifar10.edited.df %>%
      dplyr::filter(train_set_type == 'diverse') %>%
      dplyr::mutate(train_set_type = 'diverse with editing')) %>%
  dplyr::bind_rows(
    cifar10.clustered.df %>%
      dplyr::filter(train_set_type == 'random by cluster') %>%
      dplyr::mutate(train_set_type = 'by cluster')) %>%
  dplyr::group_by(train_size, train_set_type) %>%
  dplyr::summarise(mean.acc = mean(accuracy),
                   sd.acc = sd(accuracy)) %>%
